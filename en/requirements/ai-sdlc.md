...
* **Reviewing and controlling AI-generated code** — review every AI-produced diff as if it came from an unfamiliar author. Before merge, run a critic pass: a fresh agent session that gets only the spec and the diff, and is asked to find differences from the spec, not to praise the code. Give every finding an explicit verdict: accept and fix, or reject with a reason. Concrete things to check that come up on projects: the agent did not rewrite failing tests so that they pass; the tests check real flows and not just coverage; type errors were fixed at the source (for example, by updating the package the types come from), not silenced with `as any`; the change does not break something in another repository.
...
* **Cost awareness (tokenomics)** — see the price of AI work: read token and cost reports, and attribute cost to stages (spec, plan, implementation). Know that on production projects the cost of a task is estimated already at the spec / plan / task level, before implementation starts. Compare approaches by cost. Notice when a task is costing too much, so it can be stopped and re-scoped early. On projects you cannot bring your own subscription: work goes through the API with a monthly budget per developer (a few hundred dollars, on some positions 150), so the choice of model matters — simple tasks can go to a cheaper model. Where tokens really go is not vocabulary but work that should not have been done: not reading the task and confidently doing the wrong thing, experiments that get thrown away, debugging a bad first result. The factory pipeline runs every step (spec, plan, reviews, tests, implementation) and is expensive by design — do not run it for a change you can write by hand. Writing by hand sometimes is also how you keep the skill.
